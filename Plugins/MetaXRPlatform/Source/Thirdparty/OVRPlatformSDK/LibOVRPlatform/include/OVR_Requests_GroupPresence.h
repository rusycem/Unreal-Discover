...
/// ovr_GroupPresenceOptions_SetDestinationApiName() with the provided one. Use
/// this to set a user's current destination while keeping all the other Group
/// Presence parameters the same. Setting a user's destination is required to
/// enable travel from the Meta Quest Platform to your app. NOTE instead of
/// using the standalone API, we recommend setting all GroupPresence parameters
/// in one call to ovr_GroupPresence_Set(). This helps ensure that all relevant
...
/// for the provided string. Use this to set a user's current lobby session id
...
/// for the provided one. Use this to update the user's current match session
/// id while keeping all other GroupPresence parameters the same.
